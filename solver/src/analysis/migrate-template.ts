...
export function generateMigrateTemplate(
  ...
  if (auditResult.consoleViolations > 0) {
    normalizeItems.push(
      `- **console.log to structured logging:** ${auditResult.consoleViolations} violation${auditResult.consoleViolations !== 1 ? "s" : ""} to migrate to Pino or equivalent.`,
    );
  }
  if (!data.hasStructuredLogging) {
    normalizeItems.push(
      "- **Add structured logging:** Set up Pino and create lib/logger.ts.",
    );
  }
  if (!data.hasEnvValidation) {
    normalizeItems.push(
      "- **Add environment validation:** Create lib/env.ts with Zod schema for runtime env checks.",
    );
  }
  if (!data.hasErrorBoundaries) {
    normalizeItems.push(
      "- **Add error boundaries:** Create error.tsx at app root minimum.",
    );
  }
  if (!data.hasCiConfig) {
    normalizeItems.push(
      "- **Set up CI/CD pipeline:** Add automated linting, testing, and build verification.",
    );
  }
  ...
  if (data.largeFiles.length > 0) {
    debtItems.push(
      `- **Large files:** ${data.largeFiles.length} file${data.largeFiles.length !== 1 ? "s" : ""} over 400 lines. Splitting is valuable but low risk — can be addressed incrementally.`,
    );
  }
  const coveragePercent = Math.round(data.testCoverageRatio * 100);
  if (coveragePercent < 50 && data.sourceFileCount > 0) {
    debtItems.push(
      `- **Test coverage at ${coveragePercent}%:** Below the 50% target. Prioritize tests for critical business logic; legacy utility code can wait.`,
    );
  }
  if (data.approachingLargeFiles.length > 0) {
    debtItems.push(
      `- **${data.approachingLargeFiles.length} file${data.approachingLargeFiles.length !== 1 ? "s" : ""} approaching 400-line limit:** Monitor but not urgent.`,
    );
  }
